...
class pthlog
{
	...
	private static $hands = null;

	private static $player_list = null;
	private static $played_hands = null;
	private static $best_hands = null;
	private static $most_wins = null;
	private static $highest_win = null;
	private static $longest_wins = null;
	private static $longest_losses = null;
	private static $most_raises = null;
	private static $most_all_in = null;
	...
	public static function process_log($url){
		...
		self::fetch_database();
		if(!file_exists("../var/pdb/".self::$pdb)){
			return false;
		}
		self::$pdo = new PDO("sqlite:../var/pdb/".self::$pdb);
		// sqlite error handling necessary?

		self::$player_list = self::get_player_list();
		self::$hand_cash = self::get_hand_cash();
		self::$played_hands = self::get_played_hands(self::$player_list[3]);
		self::$best_hands = self::get_best_hands();
		self::$most_wins = self::get_most_wins(self::$hand_cash, self::$played_hands[2]);
		self::$highest_win = self::get_highest_win(self::$hand_cash);
		self::$longest_wins = self::get_longest_series_win(self::$hand_cash);
		self::$longest_losses = self::get_longest_series_loose(self::$player_list,self::$hand_cash);
		self::$most_raises = self::get_most_raise(self::$player_list[0],self::$player_list[1],self::$played_hands[2]);	
		self::$most_all_in = self::get_most_all_in(self::$played_hands[2]);
		self::$pot_size = self::get_pot_size();
		self::fetch_result_table_hands();
		self::create_game();
		return self::$game;
	}
	
	private static function get_pot_size(){
		$db = self::$pdo;
		$uniqueGameID = self::$game_id;
		$query = "
	...
	private static function get_most_all_in($played_hands) {
		...
		while($row = $query->fetch(PDO::FETCH_ASSOC)) {
			if(is_numeric($row['Seat']) & $row['Seat']>0 & $row['Seat']<=10) {
				...
				$most_all_in[1][] = self::replace_spec_char($row['Player']);
				$count = $row['Count'];
				if(!($count>0)) $count = 0;
	...
	private static function get_most_raise($player_list_seat,$player_list_name,$played_hands) {
	...
	private static function get_longest_series_loose($player_list,$hand_cash) {
	...
	private static function get_highest_win($hand_cash) {
			...
			if(is_numeric($row['Seat']) & $row['Seat']>0 & $row['Seat']<=10 & is_numeric($row['HandID']) & $row['HandID']>0 & $row['HandID']<count($hand_cash[$row['Seat']-1])) {
				...
				if($amount>0) {
					$highest_win[0][] = $row['Seat'];
									$highest_win[1][] = self::replace_spec_char($row['Player']);
	...
	private static function get_longest_series_win($hand_cash) {
			...
			if(is_numeric($row['Seat']) & $row['Seat']>0 & $row['Seat']<=10 & is_numeric($row['HandID']) & $row['HandID']>0) {
				...
				if($row['HandID']<count($hand_cash[$row['Seat']-1])) {
					...
					$amount_tmp = 0;
							...
							$tmp_data = array($row['HandID'],$row['Seat'],self::replace_spec_char($row['Player']),$amount_tmp);
	...
	private static function get_hand_cash() {
		$db = self::$pdo;
		$uniqueGameID = self::$game_id;
	...
	private static function create_game(){
		...
		self::$player_list = self::get_player_list();
		self::$hand_cash = self::get_hand_cash();
		self::$played_hands = self::get_played_hands($player_list["max_hand_id"]);
		self::$best_hands = self::get_best_hands();
		self::$most_wins = self::get_most_wins($hand_cash, $played_hands[2]);
		self::$highest_win = self::get_highest_win($hand_cash);
		self::$longest_wins = self::get_longest_series_win($hand_cash);
		self::$longest_losses = self::get_longest_series_loose($player_list,$hand_cash);
		self::$most_raises = self::get_most_raise(,$player_list[0],$player_list[1],$played_hands[2]);	
		self::$most_all_in = self::get_most_all_in($played_hands[2]);
		*/
		self::$game = array(
			"result" => self::$result_table,
			"player_list" => self::$player_list,
			"most hands played" => self::$played_hands,
			"best hands" => self::$best_hands,
			"most wins" => self::$most_wins,
			"highest wins" => self::$highest_win,
			"longest series of wins" => self::$longest_wins,
			"longest series of losses" => self::$longest_losses,
			"most bet/raise" => self::$most_raises,
			"most all in" => self::$most_all_in,
			"hand_cash" => self::$hand_cash,
			"pot_size" => self::$pot_size,
		);
	}
	
	private static function fetch_result_table_hands(){
		...
		WHERE a.Action IN('wins game' ,'sits out') AND a.UniqueGameID= ".self::$game_id."
		...
		self::$hands = [];
		$query = self::$pdo->prepare("SELECT * FROM Hand");
		$query->execute();
		$game = [];
		while($row = $query->fetch(PDO::FETCH_ASSOC)) {
			self::$hands[] = $row;
	...
	private static function get_played_hands($player_list_hands) {
		...
		while($row_1 = $query_1->fetch(PDO::FETCH_ASSOC)) {
			if(is_numeric($row_1['Seat']) & $row_1['Seat']>0 & $row_1['Seat']<=10) {
				...
				$played_hands[1][] = self::replace_spec_char($row_1['Player']);
	...
	private static function get_best_hands() {
		...
		for($i=1;$i<=10;$i++) {
			...
			if($row_0 = $query_0->fetch(PDO::FETCH_ASSOC)) {
				if($first_player_found) $query_1 = $query_1." UNION ";
							...
							$query_1 = $query_1."SELECT \"".self::replace_spec_char($row_0['Player'])."\" as Player, ".$i." as Seat, HandID, Seat_".$i."_Hand_int as Seat_Hand_int, Seat_".$i."_Hand_text as Seat_Hand_text FROM hand WHERE Seat_Hand_text NOT NULL AND UniqueGameID=".$uniqueGameID;
			}
		}
	...
	private static function get_player_list() {
			...
			if(is_numeric($row['Seat']) & $row['Seat']>0 & $row['Seat']<=10) {
				if(is_numeric($row['HandID']) & $row['HandID']>0 & is_numeric($row['LastCash']) & $row['LastCash'] > 0) {
					...
					if( $seat_ctr==0 | $new_player ) {
						...
						$player_list[1][$seat_ctr] = self::replace_spec_char($row['Player']);
						...
						if(!is_null($row['WinnerPlayer'])) {
							if(is_numeric($row['Eliminated']) & $row['Eliminated']==1) {
								$player_list[7][$seat_ctr][] = self::replace_spec_char($row['WinnerPlayer']);
							} else {
						...
						if(!is_null($row['WinnerPlayer'])) {
							$player_list[7][$seat_ctr-1][] = self::replace_spec_char($row['WinnerPlayer']);
						}
					}
				} else {
				...
				if(is_numeric($row['Seat']) & $row['Seat']>0 & $row['Seat']<=10) {
					...
					if($row_1 = $query_1->fetch(PDO::FETCH_ASSOC)) {
						if(is_numeric($row_1['Card_1']) & $row_1['Card_1']>=0 & $row_1['Card_1']<52 & is_numeric($row_1['Card_2']) & $row_1['Card_2']>=0 & $row_1['Card_2']<52) {
							...
							$player_list[7][$row['Seat']-1][0] = "[".self::int2string($row_1['Card_1']).",".self::int2string($row_1['Card_2'])."] ".substr($hand_text,0,$end_text);
	...
	private static function replace_spec_char($string) {
	...
	private static function int2string($int) {
	...
	private static function get_most_wins($hand_cash, $played_hands) {
		...
		while($row_1 = $query_1->fetch(PDO::FETCH_ASSOC)) {
			if(is_numeric($row_1['Seat']) & $row_1['Seat']>0 & $row_1['Seat']<=10) {
				$most_wins[0][] = $row_1['Seat'];
							$most_wins[1][] = self::replace_spec_char($row_1['Player']);
				$count = $row_1['Count'];
				if(!($count>0)) $count = 0;
				$most_wins[2][] = $count;
